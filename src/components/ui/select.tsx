...
// Context used to let the Select root communicate `indicatorPosition` down
// to SelectItem. Defaults to "left" — backoffice's only prior behavior — so
// any Select tree that doesn't opt in renders identically to before.
...
function Select({
  ...
  return (
    <SelectContext.Provider value={{ indicatorPosition }}>
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        ...
        indicatorPosition === "right" ? "ps-8 pe-2" : "pe-8 ps-2",
        className
      )}
      {...props}
    >
      <span
        className={cn(
          ...
          indicatorPosition === "right" ? "start-2" : "end-2"
        )}
      >
